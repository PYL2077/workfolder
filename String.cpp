...
String& String::erase(size_t pos, size_t len) {
    ...
    // Safely determine the actual number of characters to erase.
    // This handles both the case where 'len' would go past the end of the string
    // and the case where 'len' is npos, avoiding integer overflow.
    size_t num_to_erase = std::min(len, sz - pos);

    // If there's nothing to erase, we can return early.
    if (num_to_erase == 0) {
        return *this;
    }
    
    // 'dest' is the position where we start overwriting old characters.
    // 'src' is the position of the first character we want to keep.
    char* dest = cstr + pos;
    char* src = cstr + pos + num_to_erase;
    
    // Shift all characters from the source position leftward to the destination,
    // effectively overwriting the erased portion.
    // The loop will also copy the null terminator.
    while (*src != a_null_byte) {
        *dest++ = *src++;
    }
...
String& String::insert(size_t pos, const String& str) {
    ...
    // If inserting empty string, nothing to do
    if (str.sz == 0) {
        return *this;
    }
    
    // Shift all characters from pos onward to the right by str.sz positions
    // Work backwards to avoid overwriting data
    char* dest = cstr + sz + str.sz;  // New end position
    char* src = cstr + sz;            // Current end position
    
    // Shift characters including the null terminator
    while (src >= cstr + pos) {
        *dest = *src;
        --dest;
        --src;
    ...
    // Copy the new string into the opened space
...
String& String::replace(size_t pos, size_t len, const String& str) {
    ...
    // Strategy 1: Implement replace by shifting characters directly
    // Calculate size difference between replacement and original segment
    // size_t size_diff = str.sz;
    // size_t old_segment_len = len;
    
    if (str.sz > len) {
        // New string is longer - need to shift right
        size_t shift_amount = str.sz - len;
        
        // Shift characters after the replacement segment to the right
        char* dest = cstr + sz + shift_amount;  // New end position
        char* src = cstr + sz;                  // Current end position
        
        // Shift everything from the end of the string down to pos + len
        while (src >= cstr + pos + len) {
            *dest = *src;
            --dest;
            --src;
        }
        
        // Update size first since we've shifted
        sz += shift_amount;
        
    } else if (str.sz < len) {
        // New string is shorter - need to shift left
        size_t shift_amount = len - str.sz;
        
        // Shift characters after the replacement segment to the left
        char* dest = cstr + pos + str.sz;      // Where to start writing
        char* src = cstr + pos + len;          // First character after original segment
        
        // Shift all characters including null terminator
        while (*src != a_null_byte) {
            *dest = *src;
            ++dest;
            ++src;
        }
        *dest = a_null_byte;  // Add null terminator
...
size_t String::find_first_of(const String& str, size_t pos) {
    // If position exceeds string length, no match possible
    if (pos >= sz) {
        return npos;
    }
    
    // Search through this string starting at pos
    for (size_t i = pos; i < sz; ++i) {
        // Check if current character matches any character in str
...
size_t String::find_last_of(const String& str, size_t pos) {
    ...
    // If pos exceeds string length, search the entire string
    if (pos >= sz) {
        pos = sz - 1;
    }
    
    // Search backwards from pos to 0
    // Use a loop that handles size_t underflow properly
    for (size_t i = pos + 1; i > 0; --i) {
        size_t current_pos = i - 1;  // Actual position we're checking
        
        // Check if current character matches any character in str
        for (size_t j = 0; j < str.sz; ++j) {
            if (cstr[current_pos] == str.cstr[j]) {
